const STORAGE_KEY = "sara_promo_admin_config_v1";
...
function setStatus(text, type = "") {
  ...
  refs.status.className = `promo-status${type ? ` ${type}` : ""}`;
}
...
function buildPayloadFromForm() {
  const discountPercent = Number(refs.promoDiscount.value);
  const minOrderAmount = Number(refs.promoMinOrder.value || 0);
  const maxUsesTotal = refs.promoMaxTotal.value ? Number(refs.promoMaxTotal.value) : null;
  const maxUsesPerUser = refs.promoMaxUser.value ? Number(refs.promoMaxUser.value) : null;
...
function renderTable() {
  ...
  for (const promo of promoRows) {
    ...
    tr.innerHTML = `
      ...
      <td><div class="promo-row-actions"></div></td>
    `;

    const actions = tr.querySelector(".promo-row-actions");
...
refs.form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (promoId) {
      ...
      setStatus("Промокод обновлён.", "success");
    } else {
